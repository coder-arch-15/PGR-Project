...
"""
#cursor.execute(''' INSERT INTO HOLDINGS VALUES("aa", 100, 150, "OCDO.L", 15000)  ''')
...
for i in range(30):
	...
	print(tick)
	print(companyname)
	q= "INSERT INTO STOCKS(company,cmp,open,close,high,low,COMPANY_NAME) VALUES({0}, 0, 0, 0 , 0, 0, {1})".format(tick,companyname)
	print(q)
	cursor.execute(q)
	conn.commit()
###############################################################
